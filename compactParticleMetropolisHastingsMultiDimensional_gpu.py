...
def generateData(noObservations, initialState):
    ...
    for t in range(1, noObservations):
        ...
        state[t] = np.matmul(A,state[t - 1].T) + np.matmul(B, u) #+ epislon *randn()
        observation[t] = np.matmul(H, state[t]) #+ omega * randn()
...
def particleFilter(observations, parameters, noParticles, initialState, particleProposalDistribution, observationProposalDistribution):
    ...
    parameters_torch = torch.tensor(parameters, dtype=torch.float32).to('cuda')
    ...
    for t in range(1, noObservations):
        ...
        start = time.time()
        x_t = torch.tensor(particles[newAncestors ,t-1], dtype=torch.float32).to('cuda')
        A_t = torch.tensor(A, dtype=torch.float32).to('cuda')
        trans = torch.matmul(x_t,A_t).cpu().detach().numpy()
        B_t = torch.tensor(B, dtype=torch.float32).to('cuda')
        u = torch.matmul(B_t,parameters_torch).cpu().detach().numpy()
